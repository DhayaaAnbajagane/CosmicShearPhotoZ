...
from .DELVERunner import BinRunner, timeit

class ThreeSDirRunner(BinRunner):
    
    def __init__(self, Nsamples = 1e3, z_bin_edges = [0.0, 0.3639, 0.6143, 0.8558, 2.0], **kwargs):
    ...
    def make_3sdir_nz(self, balrog_classified_df, deep_classified_df, wide_classified_df):
        ...
        data_dir = '/scratch/midway2/dhayaa/SOMPZ'
        ...
        out_path = f'/scratch/midway2/dhayaa/SOMPZ/{redshift_file_name}/'

        if not os.path.exists(out_path):
            os.makedirs(out_path)

        ### Comment this line if you don't want to save the summary h5 file.
        save_h5 = f'/scratch/midway2/dhayaa/SOMPZ/SOMPZ_{redshift_file_name}_{today}.h5'
        ...
        #print("BINS", bins)
        #print("BINS POSTPROCESS", tomo_bins_wide_modal_even)
...
#         assert np.allclose(pc_chat_new,pc_chat)
        ...
        np.save(out_path+'nz_samples_newmethod.npy', nz_samples_newmethod)

#         with Pool(os.cpu_count()) as p:
#             nz_samples = np.concatenate(p.map(self.aux_fun_2, range(os.cpu_count())), axis=0)

#         sel = np.sum(np.isnan(nz_samples),axis=(1,2))==0
#         nz_samples = nz_samples[sel]

#         np.save(out_path+'nz_samples.npy', nz_samples)
    ...
    def return_Nzc(self, df):
        ...
        Nzc = np.zeros((len(self.zbins)-1,64*64))
    ...
    def return_Rzc(self, df):
        ...
        Rzc = np.zeros((len(self.zbins)-1,64*64))
        for index, row in redshift_sample_groupby.iterrows():
            if (index[0]<0)|(index[0]>len(self.zbins)-1): continue
    ...
    def make_nzT(self, nzti, njoin):
        ...
        for i in range(nzti.shape[1]):
            ...
            except: zmeanti[i] = np.random.randint(len(self.zbinsc))
    ...
    def return_nzsamples_fromfzt(self, fzt_dummy):
        fzt = np.zeros((4096,len(self.zbinsc))).T
    ...
    def draw_3sdir_onlyR(self,):

        ### step1
        f_T = np.random.dirichlet(self.N_T_Rsample/self.lambda_mean_R+self.alpha)

        ### step2
        f_z_T = np.array([np.random.dirichlet(x/self.lambda_T[i]+self.alpha) for i,x in enumerate(self.N_Tz_Rsample)])

        ### step3
        f_cz_Rsample = np.random.dirichlet(self.N_cz_Rsample.reshape(np.prod(self.N_cz_Rsample.shape))+self.alpha).reshape(self.N_cz_Rsample.shape)
    ...
    def draw_3sdir_newmethod(self,):
        ### step1
        f_T = np.random.dirichlet(self.N_T_Dsample/self.lambda_mean+self.alpha)

        ### step2
        f_cT = np.zeros(self.nt)
        for k in range(self.nT):
            sel = self.z2Tmap==k
            f_cT[sel] = np.random.dirichlet(self.N_Tc_Dsample[k,sel]+self.alpha) * f_T[k]

        ### step3
        f_cz = self.draw_3sdir_onlyR()
    ...
    def aux_fun(self, i):
        ...
        for i_sample in range(self.Nsamples):
            f_zt = self.draw_3sdir_newmethod()
